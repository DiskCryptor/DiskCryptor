 ...
 * abort: optional pointer to abort flag (set to 1 to cancel)
 * Returns 1 on success, 0 on failure, -1 if cancelled
 */
int dc_api dc_derive_key_um(dc_pass *password, int kdf, u8 *salt, u8 *dk, volatile LONG *abort);
 ...
 * out_kdf: receives KDF type used (optional, can be NULL)
 * out_dk: receives derived key for caching (optional, can be NULL, must be PKCS_DERIVE_MAX bytes)
 * abort: optional pointer to abort flag (set to 1 to cancel)
 * Returns ST_OK on success, ST_CANCEL if cancelled, error code otherwise
 */
